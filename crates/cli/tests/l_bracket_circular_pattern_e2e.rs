//! End-to-end subprocess test for the L-bracket circular pattern demoable:
//! the L-bracket solid (extrude + Boolean fuse) patterned around the +Z
//! axis with a non-trivial count. The slice meets the issue #259
//! demoable: "Run a circular pattern on the L-bracket solid; commit; the
...
//!
//! Pattern axis (axis_point=(0,0,0), axis_normal=(0,0,1)) with
//! angle_step = 90° (π/2) and count 4 places the copies at 0°, 90°,
//! 180°, 270° around the source; the non-coincident copies guarantee
//! the patterned BREP differs byte-for-byte from the source L-bracket.
//!
...
fn temp_root(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "threeterm-cli-l-bracket-circular-pattern-{label}-{}-{nanos}",
        std::process::id(),
    ))
}
...
#[test]
fn l_bracket_circular_pattern_commits_through_the_cli() {
    if OcctWorker::locate().is_err() {
        eprintln!(
            "l_bracket_circular_pattern_e2e: no OCCT worker binary found; set \
    ...
    extrude(
        bin,
        &root,
        "l-bracket-circular-pattern-slab",
        slab_profile,
        3.0,
    );
    ...
    extrude(
        bin,
        &root,
        "l-bracket-circular-pattern-leg",
        leg_profile,
        3.0,
    );
    ...
    let fuse_response = boolean_fuse(
        bin,
        &root,
        "l-bracket-circular-pattern",
        "l-bracket-circular-pattern-slab",
        "l-bracket-circular-pattern-leg",
    );
    let l_bracket_revision = fuse_response["revision_hash"].as_str().unwrap().to_string();

    let pattern_response = circular_pattern(
        bin,
        &root,
        "l-bracket-circular-pattern-pattern",
        "l-bracket-circular-pattern",
        "0,0,0",
        "0,0,1",
    ...
    assert_ne!(
        ...
        l_bracket_revision,
        "circular pattern must advance the revision hash"
    );

    let reloaded = Bundle::at(&root)
        .open()
        .expect("bundle reopens after circular pattern");
    assert_eq!(
    ...
    let fused_brep_bytes =
        fs::read(root.join("brep/l-bracket-circular-pattern.brep")).expect("fused BREP reads");
    let patterned_brep_bytes = fs::read(root.join("brep/l-bracket-circular-pattern-pattern.brep"))
    ...
    for line in log.lines() {
        ...
        if [
            "brep:l-bracket-circular-pattern-slab",
            "brep:l-bracket-circular-pattern-leg",
            "brep:l-bracket-circular-pattern",
            "brep:l-bracket-circular-pattern-pattern",
        ]
        .contains(&kind)
        {
            line_count += 1;
