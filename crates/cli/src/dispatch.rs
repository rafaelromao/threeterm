...
use threeterm_protocol::schema::{
    ...
    CommandId, DEFINE_COMPONENT_COMMAND_ID, EDIT_COMPONENT_PARAMETER_COMMAND_ID, EXPORT_COMMAND_ID,
    MAKE_COMPONENT_INDEPENDENT_COMMAND_ID, REDO_COMMAND_ID, REHEARSE_COMMAND_ID,
    SKETCH_SOLVE_COMMAND_ID, TRANSFORM_COMPONENT_INSTANCE_COMMAND_ID, UNDO_COMMAND_ID,
    find_by_name,
};
use threeterm_theme::{
...
fn dispatch_bracket_with_host(
    ...
    host.execute_bracket_command(bundle, bracket_id, length, width, height, thickness)
        .map_err(DispatchError::from)
}
...
fn execute_registered_with_observer(
    ...
    match result {
        ...
        Err(DispatchError::Host(error @ HostError::StaleLastValidGeometry { .. }))
...
fn emit_host_error(error: &HostError, stderr: &mut dyn Write) -> i32 {
    ...
    let detail = match error {
        ...
        HostError::InvalidReference { detail, .. } => detail.clone(),
    ...
    let (mut diagnostic, exit) = match error {
        HostError::InvalidEdit { .. } | HostError::InvalidReference { .. } => (
            threeterm_host::domain_command_diagnostic(error),
            if matches!(error, HostError::InvalidEdit { .. }) {
                EXIT_BREP_INVALID
            } else {
                EXIT_INTEGRITY_FAILURE
            },
