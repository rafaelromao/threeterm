//! Asserts the command registry's public shape: exactly one seeded entry
//! (`list`), with every required field exposed on serialization.

use serde_json::Value;
use threeterm_protocol::schema::{COMMAND_REGISTRY, CommandSchema};

#[test]
fn registry_has_exactly_one_seeded_entry() {
    assert_eq!(
        COMMAND_REGISTRY.len(),
        1,
        "this slice seeds only the `list` command; later slices extend the table"
    );
}

#[test]
fn seeded_list_entry_has_required_fields() {
    let entry: &CommandSchema = &COMMAND_REGISTRY[0];

    assert_eq!(entry.id.0, "list");
