...
case "${1:-}" in
    ...
    slow)
        THREETERM_REQUIRE_OCCT=1 THREETERM_REQUIRE_REAL_WORKER=1 \
            cargo test --workspace --jobs 1 -- --ignored --skip production_tui_ghostty_session --test-threads=1
        ;;
    e2e)
        THREETERM_REQUIRE_OCCT=1 THREETERM_REQUIRE_REAL_WORKER=1 \
            cargo test --workspace --jobs 1 -- --include-ignored --skip production_tui_ghostty_session --test-threads=1
        ;;
    *)
