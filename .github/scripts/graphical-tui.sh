...
while (($# > 0)); do
    case "$1" in
        ...
        --evidence-root)
            ...
            shift 2
            ;;
        production_tui_ghostty_session|production_tui_create_project_extrude|production_tui_keyboard_navigation|production_tui_tapered_lofted_reinforcements)
...
check_prerequisites() {
    ...
    if [[ "$TEST_ID" == 'production_tui_create_project_extrude' ||
        "$TEST_ID" == 'production_tui_tapered_lofted_reinforcements' ]]; then
...
orbit_ready() {
    ...
    if [[ "$TEST_ID" == 'production_tui_create_project_extrude' || "$TEST_ID" == 'production_tui_tapered_lofted_reinforcements' ]]; then
